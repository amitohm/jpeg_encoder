...
INT32 main(INT32 argc,INT8 *argv[])
{
	...
	UINT8 val,byte,format,optimize,subsample;
	
	INT32 i,j;
	INT32 cols,rows,depth,quality;
	
	UINT32 bufsize = 0,bytes_written = 0;
	
	UINT8 *raw_input,*jpeg_output;
	...
	//buffer mallocs
	temp = (UINT32 *)malloc(jc_info.l1_buf_size);
	if(temp==NULL)
	{
...
#endif
	temp = (UINT32 *)malloc(jc_info.l3_buf_size);
	if(temp==NULL)
	{
	...
	raw_input = (UINT8 *)malloc(sizeof(UINT8)*bufsize);
	if(raw_input == NULL)
	{
	...
	fread(raw_input,sizeof(UINT8),bufsize,fp);
	fclose(fp);
	
	jpeg_output = (UINT8 *)malloc(sizeof(UINT8)*bufsize*5);
	if(jpeg_output == NULL)
	{
	...
	if(val==JC_WRITE_HEADERS_SUCCESS)
	{
		...
		bytes_written = fwrite(jpeg_output,sizeof(UINT8),bufsize,fp);
#if DEBUG_INFO
