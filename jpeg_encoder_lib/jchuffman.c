...
GLOBAL(unsigned char) jc_huffman_seq(jcstruct_t *s,jchuffman_t *htbl,short *block)
{
    short i,temp,temp1,temp2;
    unsigned int r,nbits,code;

    unsigned char  *dct_nbits;
    unsigned char  *huffbits_table;
    unsigned short *huffcode_table;
...
{
    unsigned int huffbits,huffcode;
...
GLOBAL(unsigned char) jc_huffman_seq_dummy(jcstruct_t *s,short *block,unsigned int *dc_count,unsigned int *ac_count)
{
    short i,temp,temp1,temp2;
    unsigned int r,nbits;
...
GLOBAL(unsigned char) jc_gen_huff_table(jcinfo_t *jc_info,unsigned char table_selector)
{
    short i,j;
    short c1,c2;
    unsigned int min;

    unsigned char *nval_tbl,*val_tbl;
    ...
    jc_memset(bits,0,(sizeof(unsigned char)*33));
    jc_memset(codesize,0,(sizeof(unsigned char)*257));
    jc_memset(others,-1,(sizeof(short)*257));
