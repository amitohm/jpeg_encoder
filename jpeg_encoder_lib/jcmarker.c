...
GLOBAL(unsigned char) jc_put_markers(jcinfo_t *jc_info)
{
    unsigned char err_val,toggle,i;
    unsigned int size;
	...
	case JC_WRITE_HEADERS:
	    jc_put_next_marker(s,(unsigned char)SOI_MARKER);
	...
	case JC_ENCODE:
	    ...
	    jc_put_next_marker(s,(unsigned char)EOI_MARKER);
    ...
    jc_info->jc_output_buf_size = (unsigned int)(s->output_buf - jc_info->jc_output_buf);
...
{
    unsigned char byte,sample_precision,components_in_frame,component_selector;
    unsigned short segment_length;
    unsigned short number_of_lines, number_of_samples_per_line;
    unsigned short horizontal_sampling_factor, vertical_sampling_factor;
    unsigned short horizontal_mcus, vertical_mcus;
    unsigned int last_mcu_cols, last_mcu_rows;
    ...
    jc_put_next_marker(s,(unsigned char)SOF0_MARKER);
	...
	byte = (unsigned char)((horizontal_sampling_factor<<4) + vertical_sampling_factor);
	jc_put_8_bits(s,byte);
...
LOCAL(unsigned char) jc_put_dqt_marker(jcinfo_t *jc_info)
{
    ...
    static const unsigned char jpeg_natural_order[64] =	{
	0,  1,  8,  16, 9,  2, 3,  10,
    ...
    unsigned short i;
    unsigned short segment_length;
    unsigned short precision, table_selector;
    int  quality_factor,inverse,qval;
    unsigned char  *quantization_table;
    unsigned short *qtbl,*prev_qtbl;
	...
	switch(table_selector)
	{
		...
		s->qtbl[0] = (unsigned short *)(jcdata_l1->quantization_table[table_selector]);
		break;

	    case QTBL_CHROMINANCE_CB:
		s->qtbl_selector[1] = QTBL_CHROMINANCE_CB;
		s->qtbl_selector[2] = QTBL_CHROMINANCE_CB;
		s->qtbl[1] = (unsigned short *)(jcdata_l1->quantization_table[table_selector]);
		s->qtbl[2] = s->qtbl[1];
		break;

	    case QTBL_CHROMINANCE_CR:
		s->qtbl_selector[2] = QTBL_CHROMINANCE_CR;
		s->qtbl[2] = (unsigned short *)(jcdata_l1->quantization_table[table_selector]);
		break;
	}
	...
	jc_put_next_marker(s,(unsigned char)DQT_MARKER);

	segment_length = (64 + 1 + 2);
	jc_put_16_bits(s,segment_length);

	byte = (unsigned char)((precision<<4) + table_selector);
	jc_put_8_bits (s,byte);
	...
	for(i=0;i<64;i++)
	{
	    ...
	    qval = MAX(1,qval);
	    qval = MIN(255,qval);
	    jc_put_8_bits(s,(unsigned char)qval);

	    qval = QINVERSE(qval);
	    qtbl[jpeg_natural_order[i]] = (unsigned short)qval;
	}
...
{
    unsigned char byte,table_selector;
    unsigned char l,l1 ;
    unsigned char nsymbol,table_class,table_identifier;
    unsigned short segment_length,code;

    unsigned char *huffbits,*nval_tbl,*val_tbl;
	...
	jc_put_next_marker(s,(unsigned char)DHT_MARKER);
...
LOCAL(unsigned char) jc_put_sos_marker(jcinfo_t *jc_info)
{
    ...
    unsigned char byte,components_in_scan,component_selector;
    unsigned char dc_table_selector, ac_table_selector;
    unsigned char Ss, Se;
    unsigned char Al, Ah;
    unsigned short segment_length;
    ...
    {
	case JC_WRITE_SOS:
	    jc_put_next_marker(s,(unsigned char)SOS_MARKER);
